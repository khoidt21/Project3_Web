...
public class LoginController extends HttpServlet {
    ...
    //Khoi tao mot lan duy nhat khi start Project
    public void init(){
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(UsersData.getInstance().checkUser(u)) {
            //Login thanh cong
            UsersData.getInstance().addUsersOnline(u.getUsername());
            getServletContext().setAttribute("users", UsersData.getInstance());
            request.getSession().setAttribute("userNow", u);
            request.getSession().setAttribute("isNew", true);
            response.sendRedirect(request.getContextPath() + "/index.jsp");
        } else {
            //Kiem tra thong tin User, và kiem tra trong HashMap
